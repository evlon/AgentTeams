...
if [ "${USE_EXISTING}" = true ]; then
    ...
    if [ "${AGENTTEAMS_CI_NO_LLM:-0}" = "1" ]; then
        unset AGENTTEAMS_LLM_API_KEY
        log "CI no-LLM mode: placeholder key was used for installation only"
    fi
    log "  Admin user:     ${TEST_ADMIN_USER}"
    log "  Matrix domain:  ${TEST_MATRIX_DOMAIN}"
...
if [ "${AGENTTEAMS_CI_NO_LLM:-0}" = "1" ]; then
    log "CI no-LLM mode: skipping Manager identity setup"
else
    _setup_manager_identity
fi
...
# Lexical glob order places test-100 between test-10 and test-11. Cleanup
# must run after every selected scenario so it can inspect their resources.
for i in "${!TESTS[@]}"; do
    if [[ "${TESTS[$i]}" == */test-100-* ]]; then
...
for test_file in "${TESTS[@]}"; do
    ...
    # Only conversational scenarios consume Manager replies/session metrics.
    # Controller tests synchronize through resource/file readiness themselves.
    if [ "${AGENTTEAMS_CI_NO_LLM:-0}" != "1" ]; then
        case "${test_name}" in
            test-01-*|test-02-*|test-03-*|test-04-*|test-05-*|test-06-*|\
            test-08-*|test-09-*|test-10-*|test-11-*|test-12-*|test-13-*|\
            test-14-*|test-15-*|test-16-*|test-21-*|test-26-*)
